  ...
  constructor() {
    ...
    this.readingLevels = {
      'high-level': {
        ...
        prompt: 'Provide a 2-3 sentence summary in simple, everyday language that a typical citizen can easily understand. Focus on what this bill would actually do and how it might affect people.',
        maxLength: 200
      },
      'detailed': {
        name: 'Detailed Summary',
        prompt: 'Provide a comprehensive 4-5 sentence summary that includes key details, implications, and context. Use clear language but include important specifics that informed citizens would want to know.',
        maxLength: 400
      }
    };
  }
...
Summary (${config.maxLength} characters max):`;
    ...
    try {
      ...
      if (summary.length > config.maxLength) {
        return summary.substring(0, config.maxLength - 3) + '...';
      }
...
Topics (comma-separated):`;
  ...
  clearCache(billId = null) {
    if (billId) {
      ...
      console.log(`🗑️ Cleared cache for bill ${billId}`);
    } else {
